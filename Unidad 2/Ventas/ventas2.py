...
df = pd.read_csv('datos_groceries.csv')

# Añadir una columna 'amount' con valor 1 para cada fila
df['amount'] = 1

# Crear una tabla dinámica para contar los artículos por compra
df_pivot = df.pivot_table(index='id_compra', columns='item', values='amount', aggfunc='sum').fillna(0)
